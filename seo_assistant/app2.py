...
def main():
    ...
    menu = ["Build Database", "Search & Suggest Links"]
    ...
    if choice == "Build Database":
        ...
        if st.button("Start Scraping and Building Database"):
            ...
            # OPTIONAL: Splitting articles into chunks for better context
            with st.spinner('Processing articles into chunks...'):
                chunked_texts = []
                chunk_metadata = []
                for idx, row in df.iterrows():
                    ...
                    for i, chunk in enumerate(chunks):
                        ...
                        # Store metadata for each chunk if needed
                ...
                st.success(f"Processed into {len(chunked_texts)} text chunks.")

            with st.spinner('Generating embeddings...'):
                embeddings = generate_embeddings(chunked_texts)
            ...
            # Save chunk metadata
            chunk_metadata_df = pd.DataFrame(chunk_metadata)
            chunk_metadata_file = os.path.join(DATA_DIR, "chunk_metadata.csv")
            chunk_metadata_df.to_csv(chunk_metadata_file, index=False)
            st.success(f"Chunk metadata saved to {chunk_metadata_file}")

            st.balloons()

    elif choice == "Search & Suggest Links":
    ...
    ---
    ### SEO Assistant
    Created with ❤️ by Aleksander Skubała.
    """)
